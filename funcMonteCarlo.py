...
def funcMonteCarlo(trial, day, dfSys, dfHome, dfEV, dfSolar, dfInput, dfOutput):
    ...
    # Pass Input Values
    maxTrials = dfInput['maxTrials'][0]
    XFMR = dfInput['XFMR'][0]
    secLimit = dfInput['secLimit'][0]
    chgrRate = dfInput['chgrRate'][0]
    maxEV = dfInput['maxEV'][0]
    maxPV = dfInput['maxPV'][0]
    numHomes = dfInput['numHomes'][0]    
    ...
    dfOutput[trial][:] > sum((day_Amp_flows > secLimit).astype(int))
    
    print('\n [--- Trial: '+  trial +'CASE: ' + str(maxEV) + 'EVs ' + str(chgrRate) + 'kW chgr ' + str(maxPV) + 'PV ---]')
    
    return (day_Slack_kW_kVAR, day_P_flows, day_Amp_flows, dfOutput)
