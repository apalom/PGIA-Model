...
day_Amp_flows = np.zeros((24,len(dfSys)+1))
day_Home_kW = np.zeros((24,len(dfSys)+1))
day_EV_kW = np.zeros((24,len(dfSys)+1))
...
for hr in range(24):
    ...
    daySlack_kW_kVAR[hr,0] = sum(sum(loadHome_kW + loadEV_kW))
    daySlack_kW_kVAR[hr,1] = sum(sum(loadHome_kVAR + loadEV_kVAR))
